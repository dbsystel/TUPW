...
The HMAC needs a key that is provided by the application. So the encryption key is calculated from something that is "known" (the HMAC key), something that is in "posession" of the program (the file) and an optional "subject" string. One does not need to specify a key. It is calculated fromup to three different parts that an attacker all needs to know and that are stored in different places.
