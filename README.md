...
The idea is to store secrets in an encrypted form and to distribute the elements the key is derived from, so it is not found in a single place.
...
Both keys are generated by calculating the [HMAC](https://en.wikipedia.org/wiki/Hash-based_message_authentication_code "HMAC")-[SHA-256](https://en.wikipedia.org/wiki/SHA-2 "SHA-256") value of the contents of a file. This HMAC is not the same HMAC as the one in the encrypted form. The file should be stored in a directory that is only accessible by the user that runs the application and nobody else. It is advisable to *not* have the key file in the repository.

The HMAC needs a key that is provided by the application. So the encryption key is calculated from something that is "known" (the HMAC key), something that is in "posession" of the program (the file) and an optional "subject" string. One does not need to specify a key. It is calculated from up to three different parts that an attacker all needs to know and that are stored in different places.

So the keys are generated from the following parts:

1. The content of a file
2. The key that the program uses
3. The subject string that the program uses (optional)
...
This program just makes it harder to get at the credentials, as the key file, the program code and the subject are needed to reconstruct the encryption key. If an attacker manages to take over the server or container, he will be able to reconstruct the key. It is just a matter of time and effort. But in this case even a key management system will not help, at all.
...
Under Windows just always enclose the data in double quotes. The above example will work correctly if one specifies `"Bla"Bla"`. *Attention*: Do *not* use quotes under Windows as `'Bla"Bla'` will be transferred to the command line program as `'Bla"Bla'`, i.e. the quotes are preserved.

Under \*nix always enclose the data in single quotes or use a escape characters (\\). The above example will work correctly if one uses `"Bla\"Bla"`. *Attention*: Do *not* use double quotes if the data contains '$' characters. The shell will consider everything after the '$' sign to be the name of an environment variable which very likely will not exist and therefore be empty.

So, enclosing the data in quotes is a good idea under \*nix but a very bad idea under Windows. Enclosing the data in double quotes is a good idea under Windows but a very bad idea under \*nix.
